...
const ProgressRing = ({ percentage, colorClass }) => {
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <span className="absolute text-[10px] font-bold text-white ml-8">{percentage}%</span>
    </div>
  );
};
...
const MatchmakerCard = ({ icon: Icon, title, percentage, theme, delay }) => {
  ...
  return (
    <div className={`group flex items-center justify-between p-2 pr-4 rounded-full border backdrop-blur-md min-w-[200px] transition-all duration-300 hover:scale-105 hover:shadow-lg animate-slideUp ${themes[theme].wrapper}`} style={{ animationDelay: `${delay}ms`, animationFillMode: 'forwards', opacity: 0 }}>
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-semibold text-gray-100 whitespace-nowrap">{title}</span>
      </div>
